...
export const sendWelcomeEmail = functions.auth.user().onCreate((user) => {
  ...
  const log = () => console.log('New welcome email sent to:', email);
  return mailTransport.sendMail(mailOptions).then(log);
});
...
export const sendByeEmail = functions.auth.user().onDelete((user) => {
  ...
  const log = () => console.log('Account deletion confirmation email sent to:', email);
  return mailTransport.sendMail(mailOptions).then(log);
});
